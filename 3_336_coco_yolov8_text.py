        ...
        if img_ann:
            with open(os.path.join(output_labels_path, f"{os.path.splitext(filename)[0]}.txt"), "a") as file_object:
                for ann in img_ann:
                    ...
                    file_object.write(f"{current_cat} "+" ".join(normalized_polygon)+"\n")
...
if __name__=='__main__':
    ...
    create_yaml(input_json_path=os.path.join(base_input_path, 'train/coco_annotations.json'), 
                output_yaml_path=os.path.join(base_output_path, "data.yaml"),
                train_path="train/images", val_path="valid/images", test_path=None)